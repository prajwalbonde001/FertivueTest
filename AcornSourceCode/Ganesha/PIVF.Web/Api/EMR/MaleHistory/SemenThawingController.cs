...
{
    [Authorize]
    public class SemenThawingController : ApiController
    {
        private static Logger logger = LogManager.GetCurrentClassLogger();
        //SemenThawingService srv = new SemenThawingService();
        SemenThawingBAL srv;
        public SemenThawingController(SemenThawingBAL _srv)
        {
            srv = _srv;
        }
        [ResponseType(typeof(int))]
        [HttpPost]
        public IHttpActionResult SaveUpdate(List<SemenThawing> lstSemenFreez)
        {
            try
            {
                ...
                var Response = srv.SaveUpdate(lstSemenFreez);
                return Ok(Response);
            }
        ...
        [HttpGet]
        public IHttpActionResult GetFreezDetails(string action)
        {
            try
            {
                logger.Info("Controller Name:" + this.ControllerContext.RouteData.Values["controller"] + ",Action:" + HttpContext.Current.Request.HttpMethod +
                 ",Method:" + this.ControllerContext.RouteData.Values["action"] + ",User:{0},UnitID:{1}", GenericSP.CurrentUser.LoginName, GenericSP.CurrentUser.UnitID);
                var Response = srv.GetFreezDetails(action);
                return Ok(Response);
            }
            catch (SqlException ex)
            {
        ...
        [ResponseType(typeof(SemenFreez))]
        [HttpGet]
        public IHttpActionResult GetFreezDetailsAfterFinalize(string action, int ID, int UnitID)  //by rohini
        {
            try
            {
                logger.Info("Controller Name:" + this.ControllerContext.RouteData.Values["controller"] + ",Action:" + HttpContext.Current.Request.HttpMethod +
                 ",Method:" + this.ControllerContext.RouteData.Values["action"] + ",User:{0},UnitID:{1}", GenericSP.CurrentUser.LoginName, GenericSP.CurrentUser.UnitID);
                var Response = srv.GetFreezDetailsAfterFinalize(action,ID,UnitID);
                return Ok(Response);
            }
            catch (SqlException ex)
            {
