angular.module('PIVF').controller('day3Ctrl', function ($sce, $scope,$rootScope, RootPGInfo, $uibModalInstance, DayThreeSrv, $filter, $timeout, Common , srvCommon, AlertMessage, $uibModal, PageConfig, localStorageService, usSpinnerService) {
    $scope.CommonFiled = {};
    $scope.CoupleDetails = {};
    ...
    $scope.Transfer = 0;
    $scope.Unobserved = 0;
    $scope.disableClickDay3 = false;       //Added by Nayan Kamble on 11/06/2019
    /*START : Date */
    ...
    //========================================================================================================================================================================
    //Fill Oocyte Grid
    $scope.GetOocyteForDayThree = function () {
        ...
        debugger;
        var ResponseData = DayThreeSrv.fillDayThreeOocyteGrid($scope.CoupleDetails.FemalePatient.FemalePatientID, $scope.CoupleDetails.FemalePatient.FemalePatientUnitID, $scope.CoupleDetails.FemalePatient.TherapyID, $scope.CoupleDetails.FemalePatient.TherapyUnitID,
            $scope.CoupleDetails.FemalePatient.DonarID, DonorCycleCodeId, $scope.CoupleDetails.FemalePatient.DonarUnitID);
        debugger;
        ResponseData.then(function (Response) {
            //Fill All Master
            var ResponseData = DayThreeSrv.FillDayThreeMaster($scope.CoupleDetails.FemalePatient.FemalePatientID, $scope.CoupleDetails.FemalePatient.FemalePatientUnitID, $scope.CoupleDetails.FemalePatient.TherapyID, $scope.CoupleDetails.FemalePatient.TherapyUnitID);
               ...
               // console.log($scope.OocyteGridData);
                ...
                angular.forEach($scope.OocyteGrid, function (item) {
                    ...
                    if (item.BIOPSYDetails == undefined || item.BIOPSYDetails == null) {
                       ...
                       // console.log("Data not Present ", item.rowdisable + " is " + item.Day3Status);
                       // console.log($scope.BIOPSY);
                    }
                    else
                    {
                        for (var i = 1; i <= 23; i++) {
                            ...
                            if($scope.Cnt == 0)
                            {
                                ...
                                $scope.BIOPSY.Monosomy = '';
                                $scope.BIOPSY.Trisomy = '';
                               // $scope.BIOPSY.Day3Status = item.Day3Status;
                                item.BIOPSYDetails.push($scope.BIOPSY);
                            }
                        }

                      //  console.log("Data Present ", item.ZeroTooltip.OocyteNumber + " is " + item.DonorPGDPGS);
                       // console.log($scope.BIOPSY);
                    }
                });
        ...
        angular.forEach($filter('filter')($scope.OocyteGrid, function (d) { return d.OocyteNumber !== selectedOocyte; }), function (Oocytes1, index) {
            angular.forEach(ApplayTo, function (selectedocytes) {
                if (selectedocytes.ID == Oocytes1.OocyteNumber) {
                    ...
                    $scope.DayThreeID = Oocytes1.DayThreeID;
                    ...
                    Oocytes1.DayThreeID = $scope.DayThreeID;
        ...
        for (var i = 0; i < $scope.OocyteGrid.length; i++) {
            if (index !== i) {
                if ($scope.OocyteGrid[i].Finalize == false) {
                    ...
                    $scope.OocyteGrid[i].CleavageID = $scope.OocyteGrid[index].CleavageID;    //Added by Nayan Kamble
        ...
        for (var i = 0; i < $scope.OocyteGrid.length; i++) {
            if (index !== i) {
                if ($scope.OocyteGrid[i].rowdisable == false) {
                    //$scope.OocyteGrid[i].PNID = 0;
                    //$scope.OocyteGrid[i].FertilizationID = 0;
                    $scope.OocyteGrid[i].CleavageID = 0;    //Added by Nayan Kamble
                    $scope.OocyteGrid[i].CellStageID = 0;
    ...
    $scope.GetImg = {};
    $scope.PreviewImgDay3 = function (Imgcollection) {
        angular.element(myModalDay3).modal('show');
    ...
    $scope.ImgPreviewOk = function () {
        angular.element(myModalDay3).modal('hide');
    }
    ...
    $scope.ClearSaveDays = function ClearSaveDays() {    //Added by Nayan Kamble on 11/06/2019
        $scope.disableClickDay3 = false;
    }
    $scope.SaveDay3Process = function () {
        debugger;
        $scope.disableClickDay3 = true;     //Added by Nayan Kamble on 11/06/2019
        usSpinnerService.spin('GridSpinner');
            ...
            if ($scope.CoupleDetails.FemalePatient.IsDonor) {
                //if ($scope.MasterData.DonarLinkCycleIsAvialble > 0) {
                    ...
                    if ($scope.isvalid == false) {
                        ...
                        var ResponseData = DayThreeSrv.SaveDay3Process($scope.GetActiveOocytes);
                        debugger;
                        ...
                        ResponseData.then(function (Response) {
                            if (Response.data == 1) {
                                ...
                                //console.log($scope.GetActiveOocytes)
                                //$rootScope.BIOPSYArr.push=
    ...
    $scope.ShowPatientPopupData = function ShowPatientPopupData(Item) {
        if (Item.ZeroTooltip != null) {
            ...
            //$scope.html = '<div style=""><table style="color: #454545;font-size:14px;"><tr><td colspan="2"><b>' + Item.ZeroTooltip.CycleCode + '</b></td></tr><tr><td><b>Date Time :</b> ' + Item.ZeroTooltip.Date + ' &nbsp;' + Item.ZeroTooltip.Time + '</td><td><b>Cryo :</b></td></tr><tr><td><b>OCD :</b>' + Item.ZeroTooltip.OCD + '</td><td><b>ECD :</b>' + Item.ZeroTooltip.ECD + '</td></tr><tr><td><b>OCC :</b>' + Item.ZeroTooltip.OCC + '</td><td><b>ZP :</b>' + Item.ZeroTooltip.ZP + '</td></tr><tr><td><b>PB :</b>' + Item.ZeroTooltip.PB + '</td><td><b>PVS :</b>' + Item.ZeroTooltip.PVS + '</td></tr><tr><td><b>Cytoplasm :</b>' + Item.ZeroTooltip.Cytoplasm + '</td><td><b>Breakage :</b>' + Item.ZeroTooltip.Breakage + '</td></tr><tr><td colspan="2"><div class="inputChkbx"><b>IVM</b><input type="checkbox"  ng-model="' + Item.ZeroTooltip.IVM + '" ng-disabled="' + true + '"></div></td></tr><tr><td><b>Embryologist :</b>' + Item.ZeroTooltip.Embryologist + '</td></tr><tr><td><b>Witness :</b>' + Item.ZeroTooltip.Witness + '</td></tr></table></div>';
    ...
    $scope.ViewReport = function (item) {
        ...
        angular.element(myModalImagePDFDay3).modal('show');
    }
    //================================================================================================================================================
