angular.module('PIVF').controller('day5Ctrl', function ($sce, $scope, RootPGInfo, $uibModalInstance, DayFiveSrv, $filter, $timeout, Common , srvCommon, AlertMessage, $uibModal, PageConfig, localStorageService, usSpinnerService) {
    $scope.CommonFiled = {};
    $scope.CoupleDetails = {};
    ...
    $scope.Unobserved = 0;
    $scope.PGDPGS = 0;
    $scope.disableClickDay5 = false;        //Added by Nayan Kamble on 11/06/2019
    /*START : Date */
    ...
    //========================================================================================================================================================================
    //Fill Oocyte Grid
    $scope.GetOocyteForDayFive = function () {
        ...
        if ($scope.CoupleDetails.FemalePatient.DonorCycleCode != "") {
            ...
            DonorCycleCodeId = 0;
            $scope.CoupleDetails.FemalePatient.DonarID = 0;
            $scope.CoupleDetails.FemalePatient.DonarUnitID = 0;

        }
        var ResponseData = DayFiveSrv.fillDayFiveOocyteGrid($scope.CoupleDetails.FemalePatient.FemalePatientID, $scope.CoupleDetails.FemalePatient.FemalePatientUnitID, $scope.CoupleDetails.FemalePatient.TherapyID, $scope.CoupleDetails.FemalePatient.TherapyUnitID,
            $scope.CoupleDetails.FemalePatient.DonarID, DonorCycleCodeId, $scope.CoupleDetails.FemalePatient.DonarUnitID);
        ResponseData.then(function (Response) {
            debugger;
            //Fill All Master
            var ResponseData = DayFiveSrv.FillDayFiveMaster($scope.CoupleDetails.FemalePatient.FemalePatientID, $scope.CoupleDetails.FemalePatient.FemalePatientUnitID, $scope.CoupleDetails.FemalePatient.TherapyID, $scope.CoupleDetails.FemalePatient.TherapyUnitID);
            ResponseData.then(function (MasterResponse) {
                debugger;
                $scope.MasterData = MasterResponse.data;
                debugger;
                $scope.MasterData.ResultList = [
                    {
                        ID: 1,
                        Description: 'Normal'
                    },
                ...
                angular.forEach($scope.OocyteGridData, function (item) {
                    ...
                    if ((item.PGDPGSDate != null && DonorCycleCodeId != 0) || item.Finalize == true) {
                        item.DonorPGDPGS = true;

                    }
                    else {
                        item.DonorPGDPGS = false;
                    }
                    if ((item.BIOPSYDetails.length > 0 && DonorCycleCodeId != 0) || item.Finalize == true) {
                        item.BIOPSYDetails.DonorBiospy = true;


                    }
                    else {
                        item.BIOPSYDetails.DonorBiospy = false;
                    }
                    if (item.BIOPSYStatus == 2) {
                        item.BIOPSYDetails.DonorBiospy = true;
                        item.BIOPSYStatus=1
                    }
                    if ((item.BIOPSYStatus == 1 && DonorCycleCodeId != 0) || item.Finalize == true) {
                        item.BIOPSYDetails.DonorBiospy = true;
                        item.BIOPSYStatus = 1
                  ...
                  //  if (item.AssistedHatching == true && item.daythreegrade!= null) {
                    ///    item.DonorAssistedHatching = true;
               // }
                    debugger;

                    /*if ((item.AssistedHatching == true && DonorCycleCodeId != 0) || item.Finalize == true) {
                        item.DonorAssistedHatching = true;
                ...
                console.log($scope.OocyteGridData)
                ...
                $scope.PGDPGS = $filter('filter')($scope.OocyteGrid, function (d) { return d.PGDPGSDate !== null && d.PGDPGSTime !== null && d.PGDPGSMethodID !== 0 && d.Finalize === true; }).length;
                //Bind Selected Date 
                angular.forEach($scope.OocyteGrid, function (item) {
                    ...
                    if (item.PGDPGSTime != null) {
                        item.PGDPGSTime = new Date(item.PGDPGSTime);
                    }
                    if (item.PGDPGSDate) {
                        item.PGDPGSDate = new Date(item.PGDPGSDate);
                    }
                    if (item.Date != null) {
                     ...
                     //Add BIOPSY Details
                    if (item.BIOPSYDetails == undefined || item.BIOPSYDetails == null) {
                        item.BIOPSYDetails = [];
                        for (var i = 1; i <= 23; i++) {
                            $scope.BIOPSY = {}
                            $scope.BIOPSY.EmbryoNo = i;
                            $scope.BIOPSY.Monosomy = '';
                            $scope.BIOPSY.Trisomy = '';
                            item.BIOPSYDetails.push($scope.BIOPSY);
                        }
                    }
                    else
                    {
                        for (var i = 1; i <= 23; i++) {
                            $scope.Cnt = $filter('filter')(item.BIOPSYDetails, function (d) { return d.EmbryoNo === i }).length;
                            if($scope.Cnt == 0)
                            {
                                $scope.BIOPSY = {}
                                $scope.BIOPSY.EmbryoNo = i;
        ...
        angular.forEach($filter('filter')($scope.OocyteGrid, function (d) { return d.OocyteNumber !== selectedOocyte; }), function (Oocytes1, index) {
            angular.forEach(ApplayTo, function (selectedocytes) {
                if (selectedocytes.ID == Oocytes1.OocyteNumber) {
                    ...
                    $scope.dayFourcellsatge = Oocytes1.dayFourcellsatge;
                    $scope.dayfourgrade = Oocytes1.dayfourgrade;
                    ...
                    Oocytes1.dayFourcellsatge = $scope.dayFourcellsatge;
                    Oocytes1.dayfourgrade = $scope.dayfourgrade;
        ...
        for (var i = 0; i < $scope.OocyteGrid.length; i++) {
            if (index !== i) {
                if ($scope.OocyteGrid[i].Finalize == false) {
                    ...
                    $scope.OocyteGrid[i].BlastocystStageID = $scope.OocyteGrid[index].BlastocystStageID;
                    $scope.OocyteGrid[i].ICMID = $scope.OocyteGrid[index].ICMID;
                    $scope.OocyteGrid[i].TPDID = $scope.OocyteGrid[index].TPDID;
        ...
        for (var i = 0; i < $scope.OocyteGrid.length; i++) {
            if (index !== i) {
                if ($scope.OocyteGrid[i].rowdisable == false) {
                    //$scope.OocyteGrid[i].PNID = 0;
                    //$scope.OocyteGrid[i].FertilizationID = 0;
                    $scope.OocyteGrid[i].BlastocystStageID = 0;
                    $scope.OocyteGrid[i].ICMID = 0;
                    $scope.OocyteGrid[i].TPDID = 0;
                    $scope.OocyteGrid[i].GradeID = 0;
                    $scope.OocyteGrid[i].PlanID = 0;
    ...
    //    $scope.SelectedOocytesToApplay = $filter('filter')($scope.OocyteGrid, function (d) { return d.OocyteNumber === selectedOocyte; });
    //    $scope.SelectedOocytesToApplay[0].IsFill = true;
    //    angular.forEach($filter('filter')($scope.OocyteGrid, function (d) { return d.OocyteNumber !== selectedOocyte && d.rowdisable !== false; }), function (Oocytes1, index) {
    ...
    $scope.GetImg = {};
    $scope.PreviewImgDay5 = function (Imgcollection) {
        angular.element(myModalDay5).modal('show');
    ...
    $scope.ImgPreviewOk = function () {
        angular.element(myModalDay5).modal('hide');
    }
    ...
    $scope.ClearSaveDays = function ClearSaveDays() {    //Added by Nayan Kamble on 11/06/2019
        $scope.disableClickDay5 = false;
    }

    $scope.SaveDay5Process = function () {
        $scope.disableClickDay5 = true;      //Added by Nayan Kamble on 11/06/2019
        usSpinnerService.spin('GridSpinner');
                ...
                //if ($scope.MasterData.DonarLinkCycleIsAvialble > 0) {
                    ...
                    angular.forEach($scope.GetActiveOocytes, function (item) {
                        if (item.Finalize) {
                            ...
                            if (item.BlastocystStageID == 0 || item.BlastocystStageID == null) {
                                $scope.isvalid = true;
                                item.BlastocystStageIDInvalid = true;
                            }
                            if (item.ICMID == 0 || item.ICMID == null) {
                                $scope.isvalid = true;
                                item.ICMIDInvalid = true;
                            }
                            if (item.TPDID == 0 || item.TPDID == null) {
                                $scope.isvalid = true;
                                item.TPDIDInvalid = true;
                            }
                    ...
                    if ($scope.isvalid == false) {
                        angular.forEach($scope.GetActiveOocytes, function (item) {
                            ...
                            if (item.PGDPGSDate != null) {
                                item.PGDPGSDate = $filter('date')(item.PGDPGSDate, 'medium');
                            }
                            if (item.PGDPGSTime != null) {
                                item.PGDPGSTime = $filter('date')(item.PGDPGSTime, 'medium');
                            }
                        });
                        var ResponseData = DayFiveSrv.SaveDay5Process($scope.GetActiveOocytes);
                        ResponseData.then(function (Response) {
                            if (Response.data == 1) {
    ...
    //========================================================================================================================================================================
    $scope.setVisibility = function (Id) {
        var myEl = angular.element(document.querySelector('#data-' + Id));
        console.log("value ",$scope.DisablePGDPGS)
        myEl.removeClass('hidePgdPDSDIV');
        myEl.addClass('ShowPgdPDSDIV');
    }
    //========================================================================================================================================================================
    $scope.HidePGDPGS = function (ID) {
        var myEl = angular.element(document.querySelector('#data-' + ID));
        myEl.removeClass('ShowPgdPDSDIV');
        myEl.addClass('hidePgdPDSDIV');
    }
    //========================================================================================================================================================================
    $scope.PGDPGSOk = function (Item, ID) {
        if (Item.PGDPGSDate == undefined || Item.PGDPGSTime == undefined || Item.PGDPGSMethodID == 0 || Item.PGDPGSMethodID == null) {
            AlertMessage.error(objResource.msgTitle, "Please Fill mandatory Values");
            if (Data.ExtendPGDPGS) {
    ...
    $scope.DisableCryo = function (Data) {
        ...
        if (Data.ExtendPGDPGS && Data.PGDPGSDate == null && Data.PGDPGSTime == null && Data.PGDPGSMethodID == 0) {
            Data.DisablePlan = true;
        }
    }
    //========================================================================================================================================================================
    ...
    //========================================================================================================================================================================
    $scope.handleFileSelect = function (evt, item) {
            ...
            if (valid) {
                ...
                reader.onload = function (evt) {
                    $scope.$apply(function ($scope) {
                        item.PGDPGSDOC = evt.target.result;
                    });
                };
    ...
    $scope.ViewReport = function (item) {
        debugger;
        $scope.extn = item.PGDPGSDOC.substring(item.PGDPGSDOC.indexOf(':') + 1, item.PGDPGSDOC.indexOf('/'));
        if ($scope.extn == 'image') {
            $scope.Image = item.PGDPGSDOC;
            $scope.content = '';
        }
        else {
            $scope.content = item.PGDPGSDOC;
            $scope.Image = null;
        }
        angular.element(myModalImagePDF).modal('show');
    }
    //========================================================================================================================================================================
    $scope.ModalImagePDFCancel = function () {
        angular.element(myModalImagePDF).modal('hide');
    }
    //========================================================================================================================================================================
    ...
    //========================================================================================================================================================================
    $scope.PlanChange = function (Item) {
        if (Item.ResultID == 2) {
            Item.PlanID = 1;
        }
        else {
    ...
    //========================================================================================================================================================================
    $scope.Finalize = function(item)
    {
    debugger
        if(item.Finalize==true)
        {
            if(item.Img.model){
                if(item.Img.model.length>0)
                {}
                else
                {
                    item.Finalize=false;
                    AlertMessage.error(objResource.msgTitle, "Please upload image before Finalize");
                }
            }
            else
            {
    ...
    //========================================================================================================================================================================
    $scope.BIOPSYOk = function (Item, ID) {
        //Item.DisablePlan = false;
        var myEl = angular.element(document.querySelector('#BIOPSY-' + ID));
        myEl.removeClass('ShowPgdPDSDIV');
        myEl.addClass('hidePgdPDSDIV');
    }
    //========================================================================================================================================================================
    $scope.HideBIOPSY = function (ID) {
        var myEl = angular.element(document.querySelector('#BIOPSY-' + ID));
        myEl.removeClass('ShowPgdPDSDIV');
        myEl.addClass('hidePgdPDSDIV');
    }
    //========================================================================================================================================================================
    $scope.setVisibilityBIOPSY = function (Id) {
        var myEl = angular.element(document.querySelector('#BIOPSY-' + Id));
        myEl.removeClass('hidePgdPDSDIV');
        myEl.addClass('ShowPgdPDSDIV');
    }
    ...
    $scope.ICM = "";
    $scope.TPD = "";
    $scope.DisplayValue = function (DOMName, ItemValue, Item) {
        debugger;
        switch (DOMName) {
            case 'Blastocyst':
                $scope.Blastocyst = ItemValue;
                //$scope.GradeValue += ItemValue;
                break;
            case 'ICM':
                $scope.ICM = ItemValue;
                //$scope.GradeValue += ItemValue;
        ...
        $scope.GradeValue = $scope.Blastocyst + "-" + $scope.ICM + "-" + $scope.TPD;
        Item.GradeValue = $scope.GradeValue;
