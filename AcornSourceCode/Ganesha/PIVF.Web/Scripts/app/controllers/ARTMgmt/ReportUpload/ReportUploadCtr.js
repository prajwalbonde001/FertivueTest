'use strict';
angular.module('PIVF').controller('ReportUploadCtr', function ($scope, $uibModal, $rootScope, ReportUploadSrv, Common, srvCommon, $location, AlertMessage, $filter, swalMessages, usSpinnerService) {
    $scope.obj = {};
    ...
    $scope.GetReportList = function () {
        ...
        if (SelectDate()) {
            ...
            if (angular.isUndefined($scope.RPT.ReportCatID)) $scope.RPT.ReportCatID = 0;
            if (angular.isUndefined($scope.RPT.Name)) $scope.RPT.Name = '';
    ...
    $scope.fillCatwiseServiceList = function (catid) {
        if (catid == 4) {
            $scope.lstService = [];
            $scope.lstService.push({ ServiceID: 0, Service: 'PAC' });
        }
        else {
            var response = ReportUploadSrv.fillCatwiseServiceList(catid);
            response.then(function (resp) {
                debugger;
