...
'use strict';
angular.module('PIVF').controller('BillingCtrl', function ($rootScope, $scope,$timeout, BillingService, $location, AlertMessage, srvCommon, Common, swalMessages, $filter, usSpinnerService, localStorageService, PatientAdvanceSrv,$uibModal) {     // , PatientVisitService
    debugger;
    $scope.maxSize = 5;
    $scope.CurrentPage = 1;
...
console.log($scope.isLoadDataSuccessful)
    ...
    $scope.LoadData3 = function (i) {
  debugger
        usSpinnerService.spin('GridSpinner');
        //$scope.BillDetails.DeletedServiceList = [];
        var DeletedServiceList = [];
     
        $scope.GetPatientDetails(i.RegID, i.RegUnitID);   
        ...
        debugger
          ...
          //  $scope.FillAdvanceList();  //Added by AniketK on 29Sept2020
            ...
            $scope.PatientBillData = {};
           // PatientBillData = $rootScope.ViewPatientBill;
            $scope.PatientBillData.BillID = i.BillID;
            $scope.PatientBillData.BillUnitID = i.BillUnitID;
            //$scope.PatientBillData.ChargeID = PatientBillData.ChargeID;
            //$scope.PatientBillData.ChargeUnitID = PatientBillData.ChargeUnitID;
            //$scope.PatientBillData.PaidAmount = PatientBillData.PaidAmount;  //
            //$scope.PatientBillData.VisitID = PatientBillData.VisitID;
            ...
            //}
            usSpinnerService.spin('GridSpinner');
            var Promise = BillingService.GetSavedBillList($scope.PatientBillData.BillID, $scope.PatientBillData.BillUnitID, $scope.PatientBillData.VisitID || 0, $scope.PatientBillData.VisitUnitID || 0);
            Promise.then(function (resp) {
               debugger
                $scope.FillServiceList = resp.data;
                if ($scope.FillServiceList.length > 0) {
    ...
    $scope.fetchpreviousprescribedservices =function() {
  ...
  debugger
        ...
        var DeletedServiceList = [];
         $scope.PatientData = {}
    $scope.PatientData.PatID= $rootScope.PatientBillDetails.ID
    $scope.PatientData.PatUnitID = $rootScope.PatientBillDetails.UnitID
    $rootScope.PatientID= $rootScope.PatientBillDetails.ID
    $rootScope.PatientUnitID = $rootScope.PatientBillDetails.UnitID
               ...
               debugger
                ...
                debugger
                    for (var i = 0; i < $scope.FillServiceList.length; i++) {
                        $scope.BillDetails.SelectedOtherServiceList.push({
                            ServiceCode: $scope.FillServiceList[i].ServiceCode,
                            RateEditable: $scope.FillServiceList[i].RateEditable,
                            Service: $scope.FillServiceList[i].Service,
                            Quantity: $scope.FillServiceList[i].Quantity,
                            BaseServiceRate: $scope.FillServiceList[i].Rate,
                            ConcessionPercentage: $scope.FillServiceList[i].ConcessionPercent,
    ...
    $scope.SelectedServiceNew = function (selSer) {
       ...
       // if (checkServiceDuplicacy($scope.BillDetails.SelectedOtherServiceList, selSer)) {    //Removed as per discussion 
            $scope.BillDetails.SelectedOtherServiceList.push({
                ...
                RateEditable: selSer.RateEditable,
                MinRate: selSer.MinRate,
                MaxRate: selSer.MaxRate,
                ConcessionPercentage: 0,
                ConcessionAmount: 0,
                NetAmount: selSer.BaseServiceRate,
                PaidAmount: 0,
                BillBalanceAmount:0,
                Isfromfetchprescription:selSer.Isfromfetchprescription || false,
            ...
            console.log(">>>>>>>>>>>>>>>> Freeze flag:",$scope.disableFeeeze)
            console.log(">>>>>>>>>>>>>>>> Freeze flag:",selSer.RateEditable)
            $scope.GetDocListNew();
       // }
  ...
  $scope.updatepatientdetails = function(i) {
  debugger
    var selectedpatient = {};
    selectedpatient.ID = i.RegID;
    selectedpatient.UnitID = i.RegUnitID;
    selectedpatient.GenderID = i.GenderId;
    selectedpatient.PatientCategoryID = i.PatientCategoryID;
    selectedpatient.MRNo = i.MRNO;
    selectedpatient.PatientName = i.PatientName;
    selectedpatient.VisitID = i.VisitID;
    selectedpatient.VisitUnitID = i.VisitUnitID;
    selectedpatient.Date = new Date(i.Date.toString("MM.dd.yyyy"));

    $rootScope.PatientBillDetails = selectedpatient;
    $scope.PatientData = {}
    $scope.PatientData.PatID= $rootScope.PatientBillDetails.ID
    $scope.PatientData.PatUnitID = $rootScope.PatientBillDetails.UnitID
    ...
    // Delay redirection to ensure data is set properly
    $timeout(function() {
        $location.path('/BillList/');
    }, 200); // Small delay to allow the update
};




$scope.openredirectionforeditbill = function(i) {
     $scope.updatepatientdetails(i)
    ...
    // Use $timeout to ensure the route change is applied before calling LoadData3
    //$timeout(function() {
    //    $scope.LoadData3(i);
    //}, 0);
};
   ...
   /***************************************************************************************************************/
    ...
    $scope.TotalBaseRate = function () {
        ...
        debugger
            ...
            if (i.ConcessionPercentage == 0) {
              ...
              if(i.RateEditable === false)
              {if (i.BaseServiceRate < i.MinRate) {
        debugger
        i.BaseServiceRate = i.MinRate; // Ensure it doesn't go below the MinRate
    } else if (i.BaseServiceRate > i.MaxRate) {
    debugger
        i.BaseServiceRate = i.MaxRate; // Ensure it doesn't exceed the MaxRate
    }}
            i.IsNotFillBaseServiceRate = false;
            $scope.disableClickNewBill = false;
        }
    ...
    debugger
let paidamount = parseFloat($scope.BillDetails.OtherSrvPaymentModeList[i].PaidAmount || 0)
let advanceBalance = parseFloat($scope.BillDetails.OtherSrvPaymentModeList[i].AdvanceBalance || 0)

console.log(">>>>>>>>>>>>>>>>>>>>> paidamount",paidamount)
console.log(">>>>>>>>>>>>>>>>>>>>> paidamount",advanceBalance)
if(advanceBalance !=0){
debugger
if (paidamount >advanceBalance) {
     ...
     $scope.SaveBillNew = function () {   //
        ...
        if ($scope.btnSaveUpdate == "Update") {          ///Update
            ...
            if ($scope.BillDetails.OtherSrvPaymentModeList[i].IsAdvanceExceeded === true) {
               ...
               if ($scope.BillDetails.OtherSrvPaymentModeList[i].PaymentModeID == 12) {
                if ($scope.BillDetails.OtherSrvPaymentModeList[i].AdvanceBalance == null || $scope.BillDetails.OtherSrvPaymentModeList[i].AdvanceBalance == undefined || $scope.BillDetails.OtherSrvPaymentModeList[i].AdvanceBalance == '') {
                    AlertMessage.warning(objResource.msgTitle, "Advance Amount can't be empty!"); // Show warning for missing AdvanceBalance
                            $scope.IsNumberNotFill = true;
                    return;
                }
            }
      ...
      $scope.fetchFromPrescription = function () {
    debugger;
    if (!$rootScope.PatientID || !$rootScope.PatientUnitID) {
        alert("Patient details are missing.");
        return;
    }
    usSpinnerService.spin('GridSpinner');
    BillingService.GetServiceDetailForBill($rootScope.PatientID, $rootScope.PatientUnitID)
        .then(function (response) {
            debugger;
            if (response.data && response.data.length > 0) {
                console.log("Prescription List:", response.data);
                
                // Iterate through prescription list
                response.data.forEach(function (prescribedItem) {
                    // Check if the item is already present with Isfromfetchprescription = true
                    let isAlreadyAdded = $scope.BillDetails.SelectedOtherServiceList.some(function (existingItem) {
                        return existingItem.ServiceID === prescribedItem.Id &&
                               existingItem.ServiceCode === prescribedItem.ServiceCode &&
                               existingItem.Service === prescribedItem.ServiceName &&
                               existingItem.Isfromfetchprescription === true;
                    });

                    // If not already added, add it
                    if (!isAlreadyAdded) {
                        $scope.SelectedServiceNew({
                            Id: prescribedItem.Id,
                            ServiceCode: prescribedItem.ServiceCode,
                            Description: prescribedItem.ServiceName,
                            BaseServiceRate: prescribedItem.BaseServiceRate,
                            RateEditable: prescribedItem.RateEditable,
                            MinRate: prescribedItem.MinRate,
                            MaxRate: prescribedItem.MaxRate,
                            Isfromfetchprescription: true
                        });
                    }
                });

                AlertMessage.success(objResource.msgTitle, objResource.prescribeditemdetailsfetchedsuccessfully);
                usSpinnerService.stop('GridSpinner');

            } else {
                AlertMessage.warning(objResource.msgTitle, objResource.noprescribeitemsfound);
                usSpinnerService.stop('GridSpinner');
            }
        })
 ...
 $scope.fetchFromPrescriptionold = function () {
    debugger;
    if (!$rootScope.PatientID || !$rootScope.PatientUnitID) {
        alert("Patient details are missing.");
        return;
    ...
    $scope.SaveBill = function () {   //
        ...
        if ($scope.btnSaveUpdate == "Update") {          ///Update
            ...
            $scope.BillDetails.ChargeID = $scope.PatientBillData.ChargeID;
            $scope.BillDetails.ChargeUnitID = $scope.PatientBillData.ChargeUnitID;
             $scope.BillDetails.VisitID = $scope.PatientBillData.VisitID
             $scope.BillDetails.VisitUnitID = $scope.PatientBillData.VisitUnitID
             $scope.PatientData.VisitID = $scope.PatientBillData.VisitID
             $scope.PatientData.VisitUnitID = $scope.PatientBillData.VisitUnitID
            // $scope.BillDetails.ChargeDetailID = $scope.PatientBillData.ChargeDetailID;
            //$scope.BillDetails.ChargeDetailUnitID = $scope.PatientBillData.ChargeDetailUnitID;
           ...
           $scope.PatientData.VisitID = $rootScope.PatientBillDetails.VisitID
             $scope.PatientData.VisitUnitID = $rootScope.PatientBillDetails.VisitUnitID//Save
            if ($scope.BillDetails.IsFreezed != undefined && $scope.ValidateBill()) {
                //if ($scope.ValidateBill()) {
                $scope.disableClickNewBill = true;
    ...
    $scope.PatientData.PatID= $rootScope.PatientBillDetails.ID
    $scope.PatientData.PatUnitID = $rootScope.PatientBillDetails.UnitID
    $rootScope.UnitId = localStorageService.get("UserInfo").UnitID || 0
    $rootScope.PatientID = $rootScope.PatientBillDetails.ID
    $rootScope.PatientUnitID = $rootScope.PatientBillDetails.UnitID
        usSpinnerService.spin('GridSpinner');
...
debugger;
var responseData = BillingService.GetBalanceAdvanceAmount($rootScope.UnitId || 0,$rootScope.PatientID,$rootScope.PatientUnitID)
 responseData.then(function (response) {
    debugger
...
$scope.isPaymentModeVisible = true;
     ...
     if($scope.tempAdvanceList != undefined){
       ...
       //End :: Added by AniketK on 05Oct2020
        ...
        $scope.BillDetails.OtherSrvPaymentModeList.splice(idx, 1);
            if ($scope.BillDetails.OtherSrvPaymentModeList.length == 0) {
            //Begin :: Added by AniketK on 05Oct2020
              if ($scope.BillDetails.NetBillAmount > 0) { 
            $scope.BillDetails.OtherSrvPaymentModeList.push({
                'PaymentModeID': 0,
                'Date': '',
                'Number': '',
 ...
 swalMessages.MessageBox(
           ...
           objResource.msgDoYouWantToPayLater,
           "warning",
         function (isConfirmed) {
           if (isConfirmed) {
               try {
                console.log("isConfirmed");
                $scope.SaveBillNew(); // Call your save function
                $scope.redirect(2);
              } catch (error) {
                console.error("An error occurred:", error);

                // Close the modal with a new alert or feedback
                swalMessages.MessageBox(
                    "Error",
                    "An error occurred while updating payment details. The modal will now close.",
                    "error"
                );
            }
        } else {
       ...
       // }            //
        ...
        for (var i = 0; i < $scope.BillDetails.OtherSrvPaymentModeList.length; i++) {
            ...
            //if ($scope.BillDetails.OtherSrvPaymentModeList[i].PaidAmount < $scope.BillList[$scope.idx].BalanceAmountSelf) {
             if ($scope.BillDetails.OtherSrvPaymentModeList[i].PaymentModeID == 12) {
                if ($scope.BillDetails.OtherSrvPaymentModeList[i].AdvanceBalance == null || $scope.BillDetails.OtherSrvPaymentModeList[i].AdvanceBalance == undefined || $scope.BillDetails.OtherSrvPaymentModeList[i].AdvanceBalance == '') {
                    AlertMessage.warning(objResource.msgTitle, objResource.msgPleaseFillMandatoryFields); // Show warning for missing AdvanceBalance
                    return false;
                }
            }
...
//($scope.BillDetails.OtherSrvPaymentModeList, function (d) { return d.Date == undefined || d.Date == '' });
    ...
    $scope.SavePaymentwithConfermation = function (BillDetails){

swalMessages.MessageBox(
    objResource.msgTitle,
    objResource.msgDoYouWantToUpdatePaymentDetails,
    "warning",
    function (isConfirmed) {
        if (isConfirmed) {
            try {
                console.log("isConfirmed");
                $scope.SaveUpdatePayment(BillDetails); // Call your save function
            } catch (error) {
                console.error("An error occurred:", error);
...
$scope.UpdatePaymentwithConfermation = function (){
...
$scope.SaveBillNewwithConfarmation = function (){

swalMessages.MessageBox(
    objResource.msgTitle,
    objResource.msgDoYouWantToSaveAsDraft,
    "warning",
    function (isConfirmed) {
        if (isConfirmed) {
            try {
