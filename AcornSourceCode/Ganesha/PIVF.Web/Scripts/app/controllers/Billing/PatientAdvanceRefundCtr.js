'use strict';
angular.module('PIVF').controller('PatientAdvanceRefundCtr', function (swalMessages, $filter, $scope, $http, AlertMessage, srvCommon, $rootScope, localStorageService, Common, $location, usSpinnerService, $uibModal, PatientVisitService, PatientAdvanceRefundSrv, PatientAdvanceSrv) {
    var objResource = {};
    $scope.PatientData = {};
    ...
    $scope.PatientAdvanceRefundData.lstPatientAdvanceDetails = [];
...
if (index2 !== -1) {
   ...
   // Used for Patient Advance Refund search
    $scope.GetPatientDetails = function GetPatientDetails() {
        ...
        usSpinnerService.spin('GridSpinner');
        $scope.PatientList = [];        
        var Promise = PatientAdvanceRefundSrv.GetPatientDetails(0,0);
  ...
  if(newValue != undefined){
    if($scope.AdvanceList.length>0){
        for(var i = 0; i < $scope.AdvanceList.length; i++){
            ...
            if($scope.selectedRow == i && $scope.PatientAdvanceRefundData.RefundAmount > $scope.PatientAdvanceRefundData.AdvanceAvailableRefund){
...
//Save Patient Advance Refund
    $scope.savePatientAdvanceRefund = function savePatientAdvance(PatientAdvanceRefundData){
        ...
        if($scope.Validate(PatientAdvanceRefundData)){             
            swalMessages.MessageBox(objResource.msgTitle, "Are you sure you want to Save Refund Patient Advance?", "warning", function (isConfirmed) {
                if (isConfirmed) {
                    debugger;
                    usSpinnerService.spin('GridSpinner');
                    ...
                    for(var i = 0; i<PatientAdvanceRefundData.lstPatientAdvanceDetails.length;i++){
                        PatientAdvanceRefundData.lstPatientAdvanceDetails.splice(i, 1);
                    }
                    ...
                    for(var j = 0; j<$scope.AdvanceList.length; j++){
                        if($scope.selectedRow == j){
                            PatientAdvanceRefundData.lstPatientAdvanceDetails.push($scope.AdvanceList[j]);
                        }
                    }
                    var ResponseData = PatientAdvanceRefundSrv.savePatientAdvanceRefund($scope.PatientAdvanceRefundData);
                    ResponseData.then(function (resp) {
                        debugger;                       
                        if (resp.data == 1)  {                           
                                AlertMessage.success(objResource.msgTitle, "Refund Advanced Saved Successfully");
                                $scope.IsFormSave = true;
                                $scope.FillAdvanceList();
                                $scope.FillRefundList();
                                //$scope.PrintPatientAdvanceReceipt();
                              //  $scope.Cancel();
                                $location.path('/PatientAdvanceRefundLists/');
                         }                                                      
    ...
    $scope.FillRefundList = function FillRefundList() {
        debugger;
        usSpinnerService.spin('GridSpinner');
        var ResponseData = PatientAdvanceRefundSrv.FillRefundList($rootScope.PatientDataDetails.RegID, $rootScope.PatientDataDetails.RegUnitID);
        ResponseData.then(function (Response) {
        usSpinnerService.stop('GridSpinner');
            if (Response.data != null) {                
    ...
    $scope.SearchDataInit = function () {
        ...
        usSpinnerService.spin('GridSpinner');
            ...
            debugger
                ...
                Advance.push(null);
               // Advance.push(null);
                Advance.push('Advance');

                //Bill.push($scope.CurrentPage - 1);   ///
                var responseData = PatientAdvanceRefundSrv.GetRefundlist(Advance, $scope.CurrentPage - 1, true);
                responseData.then(function (Response) {
                debugger
...
debugger
        $location.path('/PatientAdvanceRefund/');
    };
...
$scope.populateFormData = function (selectedItem) {
    if (selectedItem.isSelected) {
        $scope.PatientAdvanceRefundData.TotalAdvance = selectedItem.Total;
        $scope.PatientAdvanceRefundData.AdvancedConsumed = selectedItem.Used;
        $scope.PatientAdvanceRefundData.TotalRefund = selectedItem.Refund;
        $scope.PatientAdvanceRefundData.AdvanceAvailableRefund = selectedItem.Balance;
    } else {
...
$scope.downloadReport = function (item) {
    ...
    $scope.GetReportData(item).then(function () {
        ...
        const options = {
    filename: 'Advance_Refund_Receipt.pdf',
...
$scope.GetReportData = function (item) {
    return new Promise((resolve, reject) => {
        debugger;
        var response = PatientAdvanceSrv.PrintAdvanceRefundReceipt(item.PatientID, item.PatientUnitID, item.RefundID);
