'use strict';
angular.module('PIVF').controller('AppointmentController', ['$scope', 'calendarConfig', 'AppointmentService', 'PatientVisitService', 'DoctorService', 'ngDialog', 'srvCommon', 'UserService', '$location', '$filter', 'AlertMessage', 'Common', '$rootScope', 'usSpinnerService', 'localStorageService',
function ($scope, calendarConfig, AppointmentService, PatientVisitService, DoctorService, ngDialog, srvCommon, UserService, $location, $filter, AlertMessage, Common, $rootScope, usSpinnerService, localStorageService) {
 ...
 $rootScope.IsFemaleActive = false;// to highlite selected patient icon on layout
    ...
    vm.selectedDoctorFirstName = '';
    vm.selectedDoctorLastName = '';
       ...
       if ($scope.searchedDateDayId === 0) {
        ...
        responseData.then(function (Response) {
            ...
            angular.forEach($scope.AppointmentDetails, function (obj) {
                ...
                console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Step  1" ,  $scope.StartTime , new Date( $scope.StartTime ) , $scope.EndTime  ,new Date( $scope.EndTime )  )
                debugger
               if ($scope.DayId == $scope.searchedDateDayId) {
                    ...
                    debugger
                     console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Step  Patient 1" ,  obj.PatientAppStartTime , new Date(obj.PatientAppStartTime) , $scope.EndTime  ,new Date(obj.PatientAppEndTime)  )
                    ...
                    for (var j = 0 ; j < vm.doctors.length ; j++) {
                        ...
                        if (vm.doctors[j].FirstName === obj.FirstName && vm.doctors[j].LastName === obj.LastName) {
                            ...
                            debugger
                            console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Step  2" ,  obj.StartTime , obj.EndTime )
                    ...
                    if (!flag) {
                        ...
                        vm.doctors.push({
                         ...
                         console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Step  3" ,  obj.StartTime , obj.EndTime , vm.doctors )
    ...
    $scope.GetDoctorName = function GetDoctorName(DOCID, FromWhere) {
        ...
        vm.selectedDoctorLastName = $scope.DoctorCategoryListForReschedule[selectedIndex].LastName;
        vm.selectedDoctorFirstName = $scope.DoctorCategoryListForReschedule[selectedIndex].FirstName;
        debugger
    ...
    $scope.LoadAppointmentTimeSlot = function LoadAppointmentTimeSlot() {
        ...
        //  vm.selectedDoctorFullName = "";
        if (vm.selectedDoctorFullName === "" || vm.selectedDoctorFirstName === "" || vm.selectedDoctorLastName === "") //  added as when clicking GetAppoinmnetslots button  doctor name gets empty.
        {
    ...
    $scope.getTimeSlots = function intervals(startString, endString) {
        ...
        console.warn(">>>>>>>>>>>>>>> -------------------------- " , startString , endString , $scope.rescheduleDate )

        var start = moment(startString, 'YYYY-MM-DD hh:mm:ss a');
        var end = moment(endString, 'YYYY-MM-DD hh:mm a');

         console.warn(">>>>>>>>>>>>>>> -------------------------- " , start , end  , $scope.rescheduleDate )
        ...
        rescheduleResponseData.then(function (RescheduleResponse) {
            ...
            if ($scope.RescheduleAppointmentDetails.length == 0) {
                ...
                angular.forEach($scope.RescheduleAppointmentDetails, function (obj) {
                    ...
                    var originalDoctorFirstName = vm.selectedDoctorFirstName || originalDoctorFullName[2];
                    var originalDoctorLastName =  vm.selectedDoctorLastName || originalDoctorFullName[4];
                ...
                angular.forEach(uniqueDates, function (obj) {
                    debugger;
                    var originalDoctorFullName = vm.selectedDoctorFullName.split(" ");
                    var originalDoctorFirstName = vm.selectedDoctorFirstName  || originalDoctorFullName[0];
                    var originalDoctorLastName = vm.selectedDoctorLastName ||  originalDoctorFullName[1] ;
    ...
    var actions = [
        {
            ...
            label: '<img src="../images/calendarReschedule.svg" alt="Reschedule">',
            onClick: function (args) {
                ...
                RescheduleCurrentEventResponse.then(function (RescheduleResponse) {
                    ...
                    $scope.lblClinicName = objResource.lblClinicName;
                    $scope.lblDepartment = objResource.lblDepartment;
                    $scope.lblDoctorName = objResource.lblDoctorName;
                    $scope.lblAppointmentDate = objResource.lblAppointmentDate;
                    $scope.lblcAppointmentTime = objResource.lblcAppointmentTime;
                    $scope.lblNew = objResource.lblNew;
                    $scope.lblExisting = objResource.lblExisting;
                    $scope.lblcNameMRNos = objResource.lblcNameMRNos;
                    $scope.lblPrefix = objResource.lblPrefix;
                    $scope.lblFirstName = objResource.lblFirstName;
                    $scope.lblDOB = objResource.lblDOB;
                    $scope.lblPatientName = objResource.lblPatientName;
                    $scope.lblcMRNos = objResource.lblcMRNos;
                    $scope.lblcAgeGenders = objResource.lblcAgeGenders;
                    $scope.lblDOB = objResource.lblDOB;
                    $scope.lblMobNo = objResource.lblMobNo;
                    $scope.lblcPleasefillmandatoryfields = objResource.lblcPleasefillmandatoryfields;


                    $scope.lblRescheduleAppointment = objResource.lblRescheduleAppointment;
                    $scope.lblYear = objResource.lblYear;
                    $scope.lblcMonthss = objResource.lblcMonthss;
                    $scope.lblFromDate = objResource.lblFromDate;
                    $scope.lblMale = objResource.lblMale;
                    $scope.lblFemale = objResource.lblFemale;
                    $scope.lblGetTimeSlot = objResource.lblGetTimeSlot;
                    $scope.lblSTime = objResource.lblSTime;
                    $scope.lblcPleasefillallmandatoryfieldss = objResource.lblcPleasefillallmandatoryfieldss;
                  


                    $scope.lblCancelAppointment = objResource.lblCancelAppointment;
                    ...
                    vm.selectedDoctorLastName = currentPatientDetailForDeletion[0].LastName;
                    vm.selectedDoctorFirstName = currentPatientDetailForDeletion[0].FirstName;
        ...
        },
        {
            ...
            label: '<img src="../images/calendarDelete.svg" alt="calendarDelete">',
            onClick: function (args) {
    ...
    $scope.FillSelectedItems = function FillSelectedItems(obj) {
       ...
       // $scope.RegisteredPatient.FirstName = ExistingPatientName[2];
    ...
    vm.rangeSelected = function (events, startDate, endDate, doctorFirstName, doctorMiddleName, doctorLastName, departmentId, departmentName, unitName, doctorAvailableStartTime, doctorAvailableEndTime, AvailablePatientNameList) {
        ...
        vm.selectedDoctorLastName = doctorFirstName;
        vm.selectedDoctorFirstName = doctorLastName;
                    ...
                    controller: function ($scope) {
                        ...
                        $scope.addNewAppointment = function (PatientName, FirstName, LastName, PatientGenderId, PatientselectedCountry, PatientMobileNumber, PatientVisitId, Remarks, selectedMRNo, PatientGenderIdError, DOB, IsNewPatient, AppointmenUnitName) {
                            ...
                            if (!$scope.isBlankField && tempFlag) {
                                ...
                                var timedifference = new Date().getTimezoneOffset();
                                var AddNewAppointmentResponse = AppointmentService.AddNewAppointment(startDate, endDate, doctorFirstName, doctorMiddleName, doctorLastName, doctorAvailableStartTime, doctorAvailableEndTime, FirstName , PatientName[1], LastName , departmentId, PatientGenderId, PatientselectedCountry, PatientMobileNumber, PatientVisitId, Remarks, selectedMRNo, DOB, AppointmenUnitName, $rootScope.RegID, $rootScope.RegUnitID, timedifference);
                    ...
                    controller: function ($scope) {
                        ...
                        $scope.addNewAppointment = function (PatientName, FirstName, LastName, PatientGenderId, PatientselectedCountry, PatientMobileNumber, PatientVisitId, Remarks, selectedMRNo, PatientGenderIdError) {
                            ...
                            if (!$scope.isBlankField) {
                                ...
                                PatientName = PatientName.split(" ");
                                var timedifference = new Date().getTimezoneOffset();
                                var AddNewAppointmentResponse = AppointmentService.AddNewAppointment(startDate, endDate, doctorFirstName, doctorMiddleName, doctorLastName, doctorAvailableStartTime, doctorAvailableEndTime, FirstName , PatientName[1], LastName , departmentId, PatientGenderId, PatientselectedCountry, PatientMobileNumber, PatientVisitId, Remarks, selectedMRNo, DOB, AppointmenUnitName,timedifference);
                                AddNewAppointmentResponse.then(function (AddResponse) {
                                    if (AddResponse.data.value == 1) {
                                        usSpinnerService.stop('GridSpinner');
                                        console.log('Added', doctorFirstName);
