...
angular.module('PIVF').controller('DailyCollectionReportCtrl', function ($scope, AlertMessage , srvCommon , Common, $rootScope, localStorageService , PatientReportsService , usSpinnerService ) {
...
$scope.getTotalInvoiceAmount = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.InvoiceAmt) || 0), 0);
};

$scope.getTotalCashAmount = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.CashAmount) || 0), 0);
};

$scope.getTotalChequeAmount = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.ChequeAmount) || 0), 0);
};

$scope.getTotalCardAmount = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.CardAmount) || 0), 0);
};

$scope.getTotalOnlineAmount = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.OnlineAmount) || 0), 0);
};

$scope.getTotalUPIAmount = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.UPI) || 0), 0);
};

$scope.getTotalAdvanceSettlement = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.AdvanceSettlement) || 0), 0);
};


$scope.getTotalOFTotal = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.TotalAmount) || 0), 0);
};



$scope.GetReportData = function (UnitID , FromDate , ToDate ){
    var response = PatientReportsService.GetDailyCollectionRpt(UnitID , FromDate , ToDate );
...
$scope.downloadReportNew = function () {
    ...
    const dynamicFilename = `DailyCollectionReport.pdf`;
    ...
    const options = {
        ...
        jsPDF: {
            unit: 'in',
            format: [30, 7], // Custom format for larger pages
...
$scope.viewReportOnNextTabAndDownload = function () {
    ...
    const options = {
        ...
        image: { type: 'jpeg', quality: 0.98 },
        html2canvas: { scale: 2 },
        jsPDF: { unit: 'in', format: [30, 8], orientation: 'landscape' },
        pagebreak: { mode: ['avoid-all', 'css', 'legacy'] }
    };
