...
angular.module('PIVF').controller('DailyRevenueReportCtrl', function ($scope, AlertMessage , srvCommon , Common, $rootScope, localStorageService , PatientReportsService , usSpinnerService ) {
...
$scope.getTotalBillAmount = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.TotalBillAmount) || 0), 0);
};

$scope.getTotalDiscount = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.Discount) || 0), 0);
};

$scope.getTotalNetBillAmount = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.NetBillAmount) || 0), 0);
};
...
$scope.GetReportData = function (UnitID , FromDate , ToDate){
    var response = PatientReportsService.GetDailyRevenueReport(UnitID , FromDate , ToDate);
...
$scope.downloadReportNewWithPreview = function () {
    ...
    const dynamicFilename = `Daily_Revenue_Report.pdf`;
    ...
    const options = {
        ...
        jsPDF: {
            unit: 'in',
            format: [15, 7.5], // Custom format for larger pages
