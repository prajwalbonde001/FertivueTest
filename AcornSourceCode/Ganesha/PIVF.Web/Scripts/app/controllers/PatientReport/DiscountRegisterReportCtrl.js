...
angular.module('PIVF').controller('DiscountRegisterReportCtrl', function ($scope, AlertMessage , srvCommon , Common, $rootScope, localStorageService , PatientReportsService , usSpinnerService ) {
...
$scope.getTotalServiceCost = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.ServiceCost) || 0), 0);
};

$scope.getTotalNetAmount = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.NetAmount) || 0), 0);
};

$scope.getTotalConcession = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.Concession) || 0), 0);
};
...
$scope.GetReportData = function (UnitID , FromDate , ToDate){
    var response = PatientReportsService.GetDiscountRegisterReport(UnitID , FromDate , ToDate);
...
$scope.downloadReportNewWithPreview = function () {
    ...
    const dynamicFilename = `Discount_Register_Report.pdf`;
...
$scope.downloadReport = function () {
    ...
    const options = {
        filename: 'Discount_Register_Report.pdf',
