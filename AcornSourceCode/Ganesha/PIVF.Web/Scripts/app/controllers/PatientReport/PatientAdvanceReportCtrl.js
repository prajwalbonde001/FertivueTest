...
angular.module('PIVF').controller('PatientAdvanceReportCtrl', function ($scope, AlertMessage , srvCommon , Common, $rootScope, localStorageService , PatientReportsService , usSpinnerService) {
...
$scope.downloadReportNewWithPreview = function () {
    ...
    if (tableFooter) {
        originalFooterStyle = tableFooter.style.cssText;
    ...
    const options = {
        ...
        html2canvas: { scale: 2 },
        jsPDF: { unit: 'in', format: [20, 9], orientation: 'landscape' },
    ...
    html2pdf()
        ...
        .then((pdf) => {
            ...
            return pdf.output('bloburl'); // Generate preview URL
        })
        .then((pdfUrl) => {
...
$scope.downloadReportNew = function () {
    ...
    const dynamicFilename = `Patient_Advance_Report.pdf`;
    ...
    const options = {
        ...
        jsPDF: {
            unit: 'in',
            format: [15, 9], // Custom format for larger pages
