          ...
          .controller('MwlCalendarDayCtrl', ["$scope", "moment", "calendarHelper", "calendarEventTitle", function ($scope, moment, calendarHelper, calendarEventTitle) {
              ...
              function refreshView() {
                ...
                //  debugger;
                  ...
                  var test = new Array();
                  angular.forEach(vm.nonAllDayEvents, function (item) {
                     // debugger;
                      if ((moment(item.event.startsAt).format('DD MMM YYYY') == moment(item.event.StartTime).format('DD MMM YYYY')) && (item.event.isCancelled == false)) {

                          if ((Date.parse(item.event.startsAt) >= Date.parse(item.event.StartTime) && Date.parse(moment(item.event.endsAt).format('DD MMM YYYY hh:mm:ss')).getTime() <= Date.parse(item.event.EndTime).getTime())) {// moment.format('DD MMM YYYY h:mm')
                              test.push(item);
                             // console.log('done', item);
                          }
                      }

                      //if (moment(obj.event.startsAt).format('DD MMM YYYY') == moment(new Date()).format('DD MMM YYYY'))
                      //    test.push(obj)
                  });

                  vm.nonAllDayEvents = test.slice();
                //  console.log(vm.nonAllDayEvents);
                  //Make left alignment 0
                  angular.forEach(vm.nonAllDayEvents, function (obj) {
                      obj.left = 0;
                  });
