angular.module('PIVF').controller('day2Ctrl', function ($sce, $scope, RootPGInfo, $uibModalInstance, DayTwoSrv, $filter, $timeout, Common , srvCommon, AlertMessage, $uibModal, PageConfig, localStorageService, usSpinnerService) {
    $scope.CoupleDetails = {};
    $scope.CommonFiled = {};
    ...
    $scope.Transfer = 0;
    $scope.Unobserved = 0;
    $scope.disableClickDay2 = false;    //Added by Nayan Kamble on 11/06/2019
    /*START : Date */
    ...
    //========================================================================================================================================================================
    //Fill Oocyte Grid
    $scope.GetOocyteForDayTwo = function () {
        ...
        var ResponseData = DayTwoSrv.fillDayTwoOocyteGrid($scope.CoupleDetails.FemalePatient.FemalePatientID, $scope.CoupleDetails.FemalePatient.FemalePatientUnitID, $scope.CoupleDetails.FemalePatient.TherapyID, $scope.CoupleDetails.FemalePatient.TherapyUnitID);
        ResponseData.then(function (Response) {
            //Fill All Master
            var ResponseData = DayTwoSrv.FillDayTwoMaster($scope.CoupleDetails.FemalePatient.FemalePatientID, $scope.CoupleDetails.FemalePatient.FemalePatientUnitID, $scope.CoupleDetails.FemalePatient.TherapyID, $scope.CoupleDetails.FemalePatient.TherapyUnitID);
    ...
    $scope.ApplayToSelectAll = function (item, index) {
         ...
         angular.forEach($filter('filter')($scope.OocyteGrid, function (d) { return d.OocyteNumber !== item.OocyteNumber && d.Finalize !== true && d.rowdisable !== true; }), function (Oocytes1, index) {
        ...
        for (var i = 0; i < $scope.OocyteGrid.length; i++) {
            if (index !== i) {
                if ( $scope.OocyteGrid[i].rowdisable == false) {
                    ...
                    $scope.OocyteGrid[i].FragmentationID = 0;
    ...
    $scope.GetImg = {};
    $scope.PreviewImgDay2 = function (Imgcollection) {
        angular.element(myModalDay2).modal('show');
    ...
    $scope.ImgPreviewOk = function () {
        angular.element(myModalDay2).modal('hide');
    }
    ...
    $scope.ClearSaveDays = function ClearSaveDays() {    //Added by Nayan Kamble on 11/06/2019
        $scope.disableClickDay2 = false;
    }
    $scope.SaveDay2Process = function () {
        $scope.disableClickDay2 = true;       //Added by Nayan Kamble on 11/06/2019
        usSpinnerService.spin('GridSpinner');
        ...
        if ($scope.CoupleDetails.FemalePatient.IsDonor) {
            //if ($scope.MasterData.DonarLinkCycleIsAvialble > 0) {
                ...
                if ($scope.isvalid == false) {
                    ...
                    var ResponseData = DayTwoSrv.SaveDay2Process($scope.GetActiveOocytes);
                    ResponseData.then(function (Response) {
                        if (Response.data == 1) {
                            usSpinnerService.stop('GridSpinner');
