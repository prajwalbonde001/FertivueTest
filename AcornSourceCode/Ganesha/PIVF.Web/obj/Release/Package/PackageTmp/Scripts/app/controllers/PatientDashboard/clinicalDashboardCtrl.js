'use strict';
angular.module('PIVF').controller("clinicalDashboardCtrl",  function ($rootScope, $scope, localStorageService, PatientDashboardSrv, Common, usSpinnerService, srvCommon, AlertMessage, $filter, $location , $uibModal) {
...
$scope.downloadReportWithPreview = function () {
    ...
    html2pdf()
        ...
        .finally(() => {
            setTimeout(() => {
                tableContainer.classList.remove('no-table-styles');
                tableContainer.classList.add('custom-table-container');
                element.classList.remove('hide-in-pdf');
            }, 1000);
        });
};
   ...
   //fetching List of GetToDoListDashboardPatientList New
    $scope.GetToDoListDashboardPatientListNew = function (fromDate = $scope.mychartFromDate , toDate = $scope.mychartToDate , Type = $scope.selectedDashboardTab) {
        debugger;
        usSpinnerService.spin('GridSpinner');
...
function formatDates(date) {
    ...
    $rootScope.patientListToDate = $scope.mychartToDate  //for PatientLIst
        ...
        $scope.GetTodoListData() // getTodoListData
        //$scope.GetFootFallData() //getFootFallData
        $scope.GetToDoListDashboardPatientListNew(); // get todoListPatientList
    }
...
$scope.showTab = function (tab) {
    ...
    if (tab === 'day') {
         ...
         $scope.showModeBtn = true;
         $scope.selectedMode = 'today'; // Default tab
         $scope.showMode($scope.selectedMode);
...
$scope.showMode = function (tab) {
    $scope.selectedMode = tab;

    const today = new Date(); // Current date

    if (tab === 'today') {
       
        console.log(">>>>>>>>>>>>> 1 Current Date:>>>>>>>", tab , formatDateForAPIcall(today))
      ...
      // For 'tommorrow', log the current date
      $scope.mychartFromDate = formatDateForAPIcall(tomorrow)
      $scope.mychartToDate = formatDateForAPIcall(tomorrow)
      ...
      console.log(">>>>>>>>>>>> 2 Tomorrow >>>>>>>>", formatDateForAPIcall(tomorrow))
    }
...
$scope.GetTodoListData = function (fromDate =  $scope.mychartFromDate , toDate = $scope.mychartToDate){
 usSpinnerService.spin('GridSpinner');
    var response = PatientDashboardSrv.GetToDoListDashboardCount( fromDate , toDate );
     response.then(function (res) {
                        $scope.TodoData = res.data.BarData[0].Data;
                        $scope.TodoLabels = res.data.BarChart; 
                  
                        $scope.changeSelectedTab($scope.TodoLabels[0] , $scope.TodoData[0] ); 
                        usSpinnerService.stop('GridSpinner');
    })
...
$scope.GetFootFallData = function (fromDate =  $scope.mychartFromDate , toDate = $scope.mychartToDate){
 usSpinnerService.spin('GridSpinner');
    var response = PatientDashboardSrv.GetFootFallDashboardCount( fromDate , toDate );
     response.then(function (res) {
                        $scope.FootFallData = res.data.BarData[0].Data;
                        $scope.FootFallLabels = res.data.BarChart; 
                        usSpinnerService.stop('GridSpinner');
    })
...
/*--------------------------------------------------------------------------*/
   ...
   $scope.ShowFootFallPatientList = function (label) {
     $rootScope.FormName = label ;
     $rootScope.btnId18 = 'footFall';
     $location.path('/PatientDashboardList/'); 
   }

  $scope.GetToDoListDashboardPatientList = function (label) {
     $rootScope.FormName = label ;
     $rootScope.btnId18 = 'todoList';
     $location.path('/PatientDashboardList/');  
  }
