...
angular.module('PIVF').controller('DailyOutstandingReportCtrl', function ($scope, AlertMessage , srvCommon , Common, $rootScope, localStorageService , PatientReportsService , usSpinnerService ) {
...
$scope.getTotalNetAmount = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.NetAmt) || 0), 0);
};

$scope.getTotalPaidAmount = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.PaidAmt) || 0), 0);
};

$scope.getTotalBalanceAmount = function () {
    return $scope.reportData.reduce((total, item) => total + (parseFloat(item.BalanceAmt) || 0), 0);
};
...
$scope.GetReportData = function (UnitID , FromDate , ToDate){
    var response = PatientReportsService.GetDailyOutstandingReport(UnitID , FromDate , ToDate);
...
$scope.downloadReportNewWithPreview = function () {
    ...
    const dynamicFilename = `Daily_Outstanding_Report.pdf`;
    ...
    const options = {
        ...
        jsPDF: {
            unit: 'in',
            format: [13, 8], // Custom format for larger pages
...
$scope.downloadReport = function () {
    ...
    const options = {
        filename: 'Daily_Outstanding_Report.pdf',
