...
angular.module('PIVF').controller('ServiceWiseBillingReportCtrl', function ($scope, AlertMessage , srvCommon , Common, $rootScope, localStorageService , PatientReportsService, usSpinnerService ) {
...
$scope.getTotalAmount = function(field) {
    return $scope.reportData.reduce(function(acc, data) {
...
$scope.GetReportData = function (UnitID , FromDate , ToDate){
    var response = PatientReportsService.GetServiceWiseBillingReport(UnitID , FromDate , ToDate);
...
$scope.downloadReportNewWithPreview = function () {
    ...
    const dynamicFilename = `Servicewise_Billing_Report.pdf`;

    const options = {
        margin: [0.60, 0.25, 0.50, 0.25],
        image: { type: 'jpeg', quality: 0.98 },
        html2canvas: { scale: 2 },
        jsPDF: { unit: 'in', format: [25, 12], orientation: 'landscape' },
...
$scope.downloadReportNew = function () {
    ...
    const options = {
        ...
        jsPDF: {
            unit: 'in',
            format: [25, 12], // Custom format for larger pages
...
$scope.downloadReport = function () {
    ...
    const options = {
        filename: 'Servicewise_Billing_Report.pdf',
        margin: [0.25, 0.25, 0.25, 0.25], // Uniform margins
        image: { type: 'jpeg', quality: 0.98 },
        html2canvas: {
            scale: 2 // Higher scale for better quality        
        },
    ...
    // Add custom footer and adjust content to fit the page width
    html2pdf()
        .set(options)
        .from(element)
        .toPdf()
        ...
        .then((pdf) => {
            ...
            // Adjust the content to fit the page width
            element.style.width = `${pageWidth * 72}px`; // Multiply by 72 to convert inches to points
